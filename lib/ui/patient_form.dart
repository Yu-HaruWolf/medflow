...
class _PatientFormScreenState extends State<PatientFormScreen> {
  ...
  // NursingPlan (簡易的な入力)
  // late TextEditingController _nursingPlanContentController; // NursingPlan を使う場合は初期化
  ...
  void initState() {
    ...
    _alcoholPerDayController = TextEditingController(); // 初期値 "0" を削除
    _cigarettsPerDayController = TextEditingController(); // 初期値 "0" を削除
    _otherSubstanceController = TextEditingController();
    _otherSubstanceRelatedInfoController = TextEditingController();

    // _nursingPlanContentController = TextEditingController(); // NursingPlan を使う場合は初期化
  }
  ...
  void dispose() {
    ...
    // _nursingPlanContentController.dispose(); // NursingPlan を使う場合は解放
  ...
  void _submitForm() async {
    // async に変更してリポジトリ呼び出しを待つ
    if (_formKey.currentState!.validate()) {
      _formKey.currentState!.save();

      final patient = Patient(
        // id: _emptyAsNull(_idController.text) ?? "auto-generated-if-empty", // IDの扱いによる
        id:
            _idController.text.trim().isEmpty
                ? "auto-generated-id-${DateTime.now().millisecondsSinceEpoch}"
      ...
      // PatientRepository を使ってデータを保存
      final patientRepository = PatientRepository();
      try {
        await patientRepository.addPatient(patient); // await を追加

        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('患者情報が作成（送信）されました')));
      } catch (e) {
        ScaffoldMessenger.of(
          context,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('患者情報入力 (任意入力)')),
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              _buildNullableCheckbox('健康管理の方法の有無', _isHealthManageMethodExist, (
                value,
              ) {
                setState(() {
