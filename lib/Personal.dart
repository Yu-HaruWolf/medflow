                  ...
                  children: <Widget>[
                    ...
                    Row(
                      children: [
                        ...
                        SizedBox(width: 20),
                        Text("性別: ${patient.gender}", style: TextStyle(fontSize: 16)),
                      ],
                    ),
                    SizedBox(height: 8),
                    // 2段目: 名前 と 生年月日
                    Row(
                      children: [
                        Text("名前: ${patient.name}", style: TextStyle(fontSize: 16)),
                        SizedBox(width: 20),
                        Text("生年月日: ${patient.birthDate}", style: TextStyle(fontSize: 16)),
                      ],
                    ),
                  ],
                ),
                ...
                itemBuilder: (context, index) {
                  return Card(
                    ...
                    child: InkWell(
                      onTap: () {
                        if (index == 0) {
                          context.read<ApplicationState>().screenId = 3; // ← 入力ページ
                        } else {
                          context.read<ApplicationState>().screenId = 4; // ← 閲覧ページ
                        }                    // 3段目: 医師ID
                      },
                      child: Container(
                        height: 60,
